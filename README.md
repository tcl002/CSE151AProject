...
To evaluate out model's fitting we looked at the training and validation error over multiple epochs. Initially after hyperparameter tuning, we found that although we were able to acheive reasonable accuricies, it appeared as though our model was overfitting. This is because while the training error continuously decreased, the validation error decreased initially but then began to increase. This suggests that the model began to fit, but after a certain point it became more accuracte with respect to the training data but less generalizable to other datasets. This was also demonstrated by the testing data which had comparitively low accuracy to the training data. 

For out final model, we again evaluated the model fitting by plotting the training and validation error over multiple epochs. This time we found that the model seemed to accuratly fit the data and was generalizable. Specifically, the change in loss with respect to the training data closely matched the change in loss with respect to the validation data. This indicates that the increase in accuracy during training was not specific to the training data, but rather indicitave of generalizable learning. Again, this was also reflected in our testing data which achieved a similar accuracy to the traning data.
